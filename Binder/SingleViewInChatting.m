...
-(void)DispSingle
{
    ...
    btnUnMatch.frame=CGRectMake(5, self.view.frame.size.height-45, (self.view.frame.size.width/2)-10, 40);
    ...
    [viewSingle addSubview:btnUnMatch];
        ...
        btnBlock.frame=CGRectMake((self.view.frame.size.width/2)+5, self.view.frame.size.height-45, (self.view.frame.size.width/2)-10, 40);
        [btnBlock setTitle:@"Block" forState:UIControlStateNormal];
        [btnBlock setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        btnBlock.titleLabel.font=fontname18;
        [btnBlock addTarget:self action:@selector(onBlockConfirm:) forControlEvents:UIControlEventTouchDown];
        [btnBlock setBackgroundColor:[UIColor colorWithRed:252/255.0 green:89/255.0 blue:77/255.0 alpha:1.0]];
        [viewSingle addSubview:btnBlock];
...
-(void)onBlock:(id)sender
{
    ...
    NSString *strValues=[NSString stringWithFormat:@"{\"id\":\"%@\",\"token\":\"%@\",\"block_id\":\"%@\"}",strid,strToken,strRecvId];
...
-(IBAction)onBlockConfirm:(id)sender
{
    NSString *strmesg=[NSString stringWithFormat:@"Are you sure want to block %@ ?",strReciverName];
